    ...
    def generate_layout_from_file(self, files, dump_path):
        for file in files:
            ...
            for annotationLine in annotationLines:
                ...
                camera_rotation = [0,0,0] #labels[15:18]
                ...
                if(object_type != "Shelf"):
                    if(object_location[0] < -25):
                        object_location[0] = -55
                    elif(object_location[0] > 0):
                        object_location[0] = 10
                    object_location[2] = 2

                if(object_type == "Shelf"):
                    object_location[0] = -65
                	
                object_location[0] += object_dimensions[0]/2
                object_location[1] += object_dimensions[1]/2
                object_location[2] += object_dimensions[2]/2
                # from CM to Meter Conversion
                object_dimensions = [float(i)/100 for i in object_dimensions]
                object_location = [float(i)/100 for i in object_location]
        ...
        for annotation in curr_annotations.values():
            if(annotation["shelf_number"] == shelfNumber):
                if(annotation["object_type"] == "Shelf"):
                    shelf = annotation
                elif(annotation["object_type"][:3] == "Box"):
