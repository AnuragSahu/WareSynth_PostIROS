...
class GenerateEgoCentricTopLayout(object):
    ...
    def writeLayout(self, ID, dump_path, shelf_and_boxes, min_shelf_number, max_shelf_number):
        ...
        for shelf_number in range(min_shelf_number, max_shelf_number+1):
            ...
            shelf_layouts[shelf_number], center_of_shelf = self.getShelfLayout(
                shelfs)
            box_layouts[shelf_number] = self.getBoxesLayouts(
                boxes, center_of_shelf)
    ...
    def saveNpyFiles(self, topEgoLayouts, ID, dump_path):
        ...
        for shelf in range(Constants.MAX_SHELVES):
            if(shelf >= len(topEgoLayouts)):
                ...
                pixels = [pixels[i * width:(i + 1) * width]
                          for i in range(height)]

            pixels = np.array(pixels)

            if(self.DEBUG):
                cv2.imwrite(filePathManager.getDebugRackLayoutPath(
                    "top", ID, shelf), pixels)
    ...
    def calculateCenter(self, shelf, boxes):
        ...
        for box in boxes:
            ...
            box["center"] = [
                float(box_center_x) - float(center_x), float(box_center_y)-float(center_y)]
        return [shelf, boxes]
    ...
    def getOneBoxLayout(self, annotation, layout, fill, center_of_shelf):
        ...
        center_x = center_of_shelf[0] - \
            self.scale*(center_of_shelf[0]-center_x)
        center_y = center_of_shelf[1] - \
            self.scale*(center_of_shelf[1]-center_y)
        ...
        rectangle = self.get_rect(center_x, int(
            self.length/self.res) - center_y, obj_l, obj_w, orient)
    ...
    def write_layouts(self, rack_layouts, box_layouts, ID, dump_path):
        ...
        for shelf in range(Constants.MAX_SHELVES):
            ...
            if(shelf not in rack_layouts):
                ...
                # pixelsb = [pixelsb[i * width:(i + 1) * width]
                #            for i in range(height)]
        ...
        for shelf in range(empty_npy):
            if(self.DEBUG):
                cv2.imwrite(filePathManager.getDebugRackLayoutPath(
                    "top", ID, write_track+shelf), empty_pixels)
                filePathManager.updateDebugImageNumber()
